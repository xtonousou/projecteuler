...
for i in xrange(len(grid)):
    for j in xrange(len(grid)):
        if j < len(grid[j]) - 3:
            product = max(product, int(grid[i][j]) * int(grid[i][j + 1]) * int(grid[i][j + 2]) * int(grid[i][j + 3]))
        if i < len(grid[i]) - 3:
            product = max(product, int(grid[i][j]) * int(grid[i + 1][j]) * int(grid[i + 2][j]) * int(grid[i + 3][j]))
            if j < len(grid[j]) - 3:
                product = max(product, int(grid[i][j]) * int(grid[i + 1][j + 1]) * int(grid[i + 2][j + 2]) * int(grid[i + 3][j + 3]))
            if j > 3:
                product = max(product, int(grid[i][j]) * int(grid[i + 1][j - 1]) * int(grid[i + 2][j - 2]) * int(grid[i + 3][j - 3]))
